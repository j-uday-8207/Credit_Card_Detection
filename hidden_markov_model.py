...
class HMM:
    ...
    def train_model(self, observations, steps):
        ...
        observation_chunks = [observations[i:min(i + chunk_size, num_observations)] for i in range(0, num_observations, chunk_size)]
        num_chunks = len(observation_chunks)
        # Main loop for given steps
        for _ in range(steps):
            pi_prob = np.zeros(self.n_states)
            # transition_prob = np.zeros((self.n_states, self.n_states))
            # emission_prob = np.zeros((self.n_states, self.n_possible_observations))
            num1  = np.zeros((self.n_states,self.n_states))
            denom1 = np.zeros((self.n_states,self.n_states))
            num2 = np.zeros((self.n_states,self.n_states))
            denom2 = np.zeros((self.n_states,self.n_states))
            # num2,denom2 = 0,0
            ...
            for chunk in observation_chunks:
                ...
                fwd = self.forward_process(chunk)
                bwd = self.backward_process(chunk)

                # Re-estimating of initial state probabilities
                for i in range(self.n_states):
                    self.pi_prob[i] += self.calculate_gamma(i, 0, fwd, bwd)
                ...
                for i in range(self.n_states):
                    for j in range(self.n_states):
                        ...
                        for t in range(len(chunk)):
                            num1[i][j] += self.calculate_path_probability(t, i, j, chunk, fwd, bwd)
                            denom1[i][j] += self.calculate_gamma(i, t, fwd, bwd)
            ...
            for i in range(self.n_states):
                self.pi_prob[i]/= total_valid_chunks
                for j in range(self.n_states):
                    self.transition_prob[i][j] = divide(num1[i][j],denom1[i][j])
            # self.pi_prob = pi_prob
                for k in range(self.n_possible_observations):
                    self.emission_prob[i][k] = divide(num2[i][k],denom2[i][k])
            # self.transition_prob = divide(num1,denom1)
            # self.emission_prob = divide(num2,denom2)
    ...
    def detect_fraud(self, observations, new_observation, threshold):
        ...
        delta = delta /(alpha_1[0] + 0.000001) 
